...
#define VECTOR_FOR_EACH(VEC, ITR)                                                        \
    for (auto ITR = VECTOR_BEGIN(VEC); ITR != VECTOR_END(VEC); ITR = VECTOR_NEXT(ITR))
...
#define VECTOR_FIND(VEC, VAL)                                                            \
    ({                                                                                   \
        typeof(VEC)* _vp = &(VEC);                                                       \
        static_assert(__builtin_types_compatible_p(typeof(*_vp->data), typeof(VAL)),     \
                      "Type mismatch");                                                  \
                                                                                         \
...
#define SET_DECLARE(TYPE, HASH, EQ)                                                      \
    ...
    {                                                                                    \
        if (_SET_CONTAINS_FN(TYPE)(s, v))                                                \
            return false;                                                                \
        if (!s->capacity || (float)s->size / (float)s->capacity >= SET_RESIZE_THRESHOLD) \
...
#define _MAP_INSERT_FN(KT, VT) ZAZ_CAT(__map_insert_, ZAZ_CAT(KT, ZAZ_CAT(_, VT)))
#define _MAP_CONTAINS_FN(KT, VT) ZAZ_CAT(__map_contains_, ZAZ_CAT(KT, ZAZ_CAT(_, VT)))
#define _MAP_GET_FN(KT, VT) ZAZ_CAT(__map_get_, ZAZ_CAT(KT, ZAZ_CAT(_, VT)))
#define _MAP_REMOVE_FN(KT, VT) ZAZ_CAT(__map_remove_, ZAZ_CAT(KT, ZAZ_CAT(_, VT)))
#define _MAP_SET_FN(KT, VT) ZAZ_CAT(__map_set_, ZAZ_CAT(KT, ZAZ_CAT(_, VT)))
#define _MAP_RESIZE_FN(KT, VT) ZAZ_CAT(__map_resize_, ZAZ_CAT(KT, ZAZ_CAT(_, VT)))
...
#define MAP_DECLARE(KT, VT, HASH, EQ)                                                    \
    ...
    {                                                                                    \
        if (_MAP_CONTAINS_FN(KT, VT)(m, k))                                              \
            return false;                                                                \
        if (!m->capacity || (float)m->size / (float)m->capacity >= MAP_RESIZE_THRESHOLD) \
